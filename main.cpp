...
std::unique_ptr<std::unordered_map<dpp::snowflake, std::future<void>>> sender = std::make_unique<std::unordered_map<dpp::snowflake, std::future<void>>>();
...
int main()
{
	...
	bot->on_message_create([](const dpp::message_create_t& event) {
		if (std::ranges::none_of(commands, [&event](const std::string& command) { return event.msg.content.find(command) not_eq -1; }) or
			std::ranges::count_if(*sender, [&event](const auto& element) { return element.first == event.msg.member.user_id; }) or
