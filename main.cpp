#include <dpp/dpp.h>
std::unique_ptr<dpp::cluster> bot = std::make_unique<dpp::cluster>("MTAwNDUxNDkzNTA1OTAwNTQ3MA.G2ZpBb.9dy2oNVvwI9Uy79o_LBnB4aMUf9EBdsMiCn1Dg", dpp::i_all_intents);
	...
	std::async(std::launch::async, std::function<void()>([&event]()
		{
			...
			if (event->command.get_command_name() == "gcreate") {
				giveaway gw = {
					...
					time(0), event->command.member.user_id, {},
					get<int64_t>(event->get_parameter("winners"))
				};
...
int main()
{
	...
	bot->on_slashcommand([](const dpp::slashcommand_t& event) {
		...
		if (cmd_sender->contains(event.command.member.user_id)) event.reply();
		else cmd_sender->try_emplace(event.command.member.user_id, std::async(std::launch::async, release_command, std::move(_event)));
		});
	bot->on_button_click([](const dpp::button_click_t& event) {
		std::unique_ptr<dpp::button_click_t> _event = std::make_unique<dpp::button_click_t>(std::move(event));
		if (btn_sender->contains(event.command.member.user_id)) event.reply();
		else btn_sender->try_emplace(event.command.member.user_id, std::async(std::launch::async, button_pressed, std::move(_event)));
		});
