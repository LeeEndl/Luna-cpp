...
void release_command(const dpp::slashcommand_t& event)
{
	std::async(std::launch::async, std::function<void()>([&event]()
		{
			if (event.command.get_command_name() == "purge" and event.command.member.get_user()->get_permission(event.command.guild_id) & dpp::p_manage_messages) {
				...
				int amount = get<int64_t>(event.get_parameter("amount"));
				bot->messages_get(event.command.channel_id, 0, event.command.id, 0, std::clamp(amount, 1, 100), [&](const dpp::confirmation_callback_t& mg_cb) {
					bot->message_delete_bulk(std::get<dpp::message_map>(mg_cb.value), event.command.channel_id, [&event, mg_cb](const dpp::confirmation_callback_t& mdb_cb) {
						...
						event.edit_original_response(dpp::message(what).set_flags(dpp::message_flags::m_ephemeral));
						});
					});
			}
			if (event.command.get_command_name() == "gcreate") {
				giveaway gw = { time(0), event.command.member.user_id, std::vector<dpp::snowflake>(), 1 };
				bot->message_create(dpp::message(event.command.channel_id, dpp::embed()
					...
					.set_timestamp(time(0))).add_component(dpp::component()
						.add_component(dpp::component()
							.set_emoji(u8"🎉")
							.set_id(std::format(".giveaway.{0}", _giveaway->size())))));
...
int main()
{
	...
	bot->on_ready([](const dpp::ready_t& event) {
		...
		cmds.emplace_back(dpp::slashcommand().set_name("purge")
			.set_description("mass delete messages")
			.add_option(dpp::command_option(dpp::co_integer, "amount", "amount of messages to delete", true)));
		cmds.emplace_back(dpp::slashcommand().set_name("gcreate")
			.set_description("create a giveaway"));
		if (dpp::run_once<struct bulk_command_create_t>()) bot->global_bulk_command_create(cmds);
		});
