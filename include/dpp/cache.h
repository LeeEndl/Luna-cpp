...
#define cache_decl(type, setter, getter, counter) /** Find an object in the cache by id. @return type* Pointer to the object or nullptr when it's not found */  class type * setter (snowflake id);  cache<class type> * getter (); /** Get the amount of cached type objects. */  uint64_t counter ();
