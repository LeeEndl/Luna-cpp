...
namespace dpp {
	...
	namespace utility {
		 ...
		 */
		std::string  timestamp(time_t ts, time_format tf = tf_short_datetime);
	 ...
	 */
		 ...
		 */
		std::string  markdown_escape(const std::string& text, bool escape_code_blocks = false);
		...
		*/
		std::string  emoji_mention(std::string_view name, snowflake id, bool is_animated = false);
		...
		*/
		 ...
		 */
		template <typename T>
		extern std::enable_if_t<std::is_same_v<T, image_type>, std::string>  mime_type(T type);
		 ...
		 */
		template <typename T>
		extern std::enable_if_t<std::is_same_v<T, sticker_format>, std::string>  mime_type(T format);
		 ...
		 */
		template <typename T>
		extern std::enable_if_t<std::is_same_v<T, image_type>, std::string>  file_extension(T type);
